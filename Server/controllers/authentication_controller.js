...
import { sendverificationOtp } from '../utills/sendverificationOtp.js';
...
export const register = async (req, res) => {
    ...
    try {
        ...
        const hashedPassword = await bcryptjs.hash(password, 10);
                    ...
                    // Send OTP
            const otpResponse = await sendverificationOtp(phone, generatedOTP);

            // Debugging log
            console.log("OTP API Response:", otpResponse);

            // Check OTP response safely
            if (otpResponse && otpResponse.response_code && otpResponse.response_code === 202) {
                ...
                return res.status(500).json({
                    ...
                    error: otpResponse || "No response from SMS API",
                });
            }
...
export const updateProfile = async (req, res) => {
    ...
    const { name, phone, email, password, distric, photoUrl } = req.body;

    // If no data to update
    if (!name && !distric && !phone && !email && !password && !photoUrl) {
    ...
    try {
        ...
        if (distric) user.distric = distric;
        if (password) {
            // Add logic to hash the new password before saving it
        ...
        // Handle JWT errors, other potential errors
